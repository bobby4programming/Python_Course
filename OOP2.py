...
class Current(Account):
    ...
    self.min_acct_bal = 10000
    
    def deactivate_the_accounnt(self):
        self.close_the_account = True
        return delattr(Account, Current)
        Account.customers -= 1#activate disable mode for the accout
    
    def withdraw(self, amount_to_withdraw, emergency=False, close_the_account=False):
        if(self.amount - amount_to_withdraw  <= 10,000):
            return "You dont have enough money in this account"
        self.amount -= float(amount_to_withdraw)
        return self.amount

        emergency = True     
        if(self.amount - amount_to_withdraw < = 10,000) or (self.amount - amount_to_withdraw > = 1000)
            self.amount -= float(amount_to_withdraw)
            return self.amount

         
    def close_the_account(self,close_the_account = True):
        if(self.amount - self.closedBal >= 1000)
            self.amount -= self.closedBal
            return deactivate_the_accounnt()
