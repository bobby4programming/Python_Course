...
class Current(Account):
    ...
    def deactivate_the_account(self,close_the_account = True):
        return delattr(self)
        #activate disable mode for the account
    
    def close_account(self,close_the_account = True,closedBal = 1000):
        return self.deactivate_the_account()
        
            

    def withdraw(self, amount_to_withdraw, emergency=False):
        if(amount_to_withdraw > self.amount) or (self.amount - amount_to_withdraw  <= 0):
            return "You don't have enough money on this account"
        elif(emergency) and (self.amount - amount_to_withdraw  <= 10000):
            self.amount -= float(amount_to_withdraw)
            return amount_to_withdraw    
...
#print('emergency withdrawal',sandra_account.withdraw(100000, emergency=True))
#print('withdrawal by Sandra',sandra_account.withdraw(78993))
#print('Sandra\'s account balance',sandra_account.balance())
#print(Account.get_num_of_customers())
#print(sandra_account.close_account,'Account closed')
#print(sandra_account)
